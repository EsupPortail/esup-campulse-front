...
<template>
    <QForm class="q-gutter-md" @submit="resetConfirm">
        <QInput v-model="newPassword.newPassword1" :label="t('forms.new-password')"
            :rules="[val => val && val.length > 0 || t('forms.required-new-password')]" filled lazy-rules
            type="password" />
        <QInput v-model="newPassword.newPassword2" :label="t('forms.repeat-new-password')" :rules="[
        ...
        ]" filled lazy-rules type="password" />
